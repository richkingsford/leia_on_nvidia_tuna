#!/usr/bin/env python3
"""Run one honest empty Step 1 reset from the current pose, then exit.

Matches the reset the modified-E2E trial performs internally
(``follow._run_reset_sequence(..., honest_step1_reset=True)``).
"""
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"[RESET] success={ok} reason={reason} "
            f"dist={_fmt(reading, 'dist_mm')}mm "
            f"x={_fmt(reading, 'x_mm', True)}mm "
            f"y={_fmt(reading, 'y_mm', True)}mm",
            flush=True,
        )
        print("[RESET] DONE_OK" if ok else "[RESET] DONE_NOT_HONEST", flush=True)
