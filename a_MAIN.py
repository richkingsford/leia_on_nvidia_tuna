...
def _clear_manual_motion_state_locked(app_state):
...
def _stop_manual_stream_server(app_state):
    server = getattr(app_state, "stream_server", None)
    app_state.stream_server = None
...
def _start_manual_stream_server(app_state, *, open_browser=False):
    if not bool(getattr(app_state, "stream_requested", True)):
        app_state.stream_enabled = False
        return False
    try:
    ...
    app_state.stream_server = stream_server
    app_state.stream_enabled = True
    ...
    app_state.pending_stream_started_url = str(url)
    log_line(f"[VISION] Livestream URL: {url}")
    if bool(open_browser):
        _open_stream_in_chrome(url)
    return True


def _suspend_manual_runtime_for_calibration(app_state):
    with app_state.lock:
        if bool(app_state.auto_running):
            return False, "[CALIBRATE] Finish or cancel the current auto-step before entering calibration mode."
        if app_state.auto_request is not None:
            return False, "[CALIBRATE] Clear the queued auto-step before entering calibration mode."
...
def _resume_manual_runtime_after_calibration(app_state, borrowed_runtime: dict | None = None):
    ...
    try:
        ...
        log_line(f"[CALIBRATE] Failed to reopen robot controller: {exc}")
        app_state.robot = None
    ...
    try:
        ...
        _set_stream_state_cyan_visibility(app_state, visibility_key)
        log_line(
            f"[VISION] Active mode: {_vision_mode_label(vision_mode)} "
            f"(--vision {_vision_mode_cli_value(vision_mode)})"
        )
        if vision_mode == VISION_MODE_CYAN and profile_applied:
            log_line(
                "[VISION] Cyan config: "
    ...
    # Stream server was kept alive during calibration; nothing to restart here.
    refresh_world_model_from_demos(app_state, force=True, min_interval_s=0.0)
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--no-stream", dest="stream", action="store_false", help="Disable livestreaming")
    ...
    if bool(args.stream):
        _start_manual_stream_server(state, open_browser=True)
    else:
    ...
    try:
        ...
        state.running = False
        _stop_manual_stream_server(state)
        if state.robot:
            state.robot.close()
        if state.vision:
            state.vision.close()
