#!/usr/bin/env python3
"""Interactive calibration launcher for motion curve discovery.

This launcher discovers how long motor commands take to achieve distance/position movements.
Output: duration curves mapping movement distance to motor runtime.

For telemetry/sensor value calibration (detecting actual distances/positions from sensor readings),
use calibrate_dist_x_y_telemetry.py instead.
"""
...
def run_guided_distance_calibration() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Primary trial count to run at each checkpoint.",
    )
...
def _interactive_trial_args(option: CalibrateOption, passthrough_args: list[str]) -> list[str]:
    ...
    print("\nTrial Setup")
    print("-----------")
    experimental_trials = _prompt_int_value("Experimental Trials: ", minimum=1)
    repeat_trials = _prompt_int_value("Repeat Trials: ", minimum=0)
    return [
        "--trials",
        str(int(experimental_trials)),
        "--repeat-trials",
        str(int(repeat_trials)),
    ]
...
def main() -> int:
    ...
    if args.choice is not None:
        ...
        if selected is None:
            ...
            return 0
        passthrough_args = list(passthrough_args) + _interactive_trial_args(selected, list(passthrough_args))
