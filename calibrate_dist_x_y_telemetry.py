#!/usr/bin/env python3
"""Interactive dist/x/y telemetry calibration using manual hotkey positioning.

This script does not command robot movement. The operator uses the normal hotkeys,
then captures telemetry and types the expected real-world value for each point.
"""
...
def _upsert_staging_telemetry_calibration(
    *,
    ...
    by_variable[str(variable)] = dict(run_record)
...
def _prompt_with_hotkeys_and_live_refresh(
    *,
    ...
    if stream is None or not hasattr(stream, "fileno") or not callable(hotkey_handler):
        return _prompt_with_live_refresh(
            prompt=str(prompt),
            vision=vision,
            world=world,
            enable_live_refresh=bool(enable_live_refresh),
                    ...
                    if bool(enable_live_refresh):
                        ...
                        if callable(live_refresh_callback):
                            try:
                                ...
                                pass
                    print(str(prompt) + str(line_buffer), end="", flush=True)
                    continue
                line_buffer += str(key)
        ...
        while True:
            ...
            while True:
                ...
                if target_hint is None:
                    entry_prompt = (
                        f"Use hotkeys to move; type expected {variable} in mm and press Enter to capture [q=quit]: "
                    )
                else:
                    entry_prompt = (
                        f"Use hotkeys to move; type expected {variable} in mm and press Enter to capture "
                        f"[Enter={target_hint:.1f}, q=quit]: "
                    )
                expected_text = _prompt_with_hotkeys_and_live_refresh(
                    prompt=entry_prompt,
                    vision=vision,
                    world=world,
                    enable_live_refresh=bool(args.livestream),
...
def _stream_preview_from_passthrough(passthrough_args: list[str]) -> tuple[bool, str]:
    ...
    parser.add_argument("--stream-host", type=str, default="127.0.0.1")
    parser.add_argument("--stream-port", type=int, default=5000)
    args, _unknown = parser.parse_known_args(list(passthrough_args or []))
    enabled = bool(args.livestream)
    url = format_stream_url(str(args.stream_host), int(args.stream_port))
    return enabled, str(url)
...
def main() -> int:
    ...
    stream_enabled, stream_url = _stream_preview_from_passthrough(passthrough_args)
    if stream_enabled:
        print(
            "[CALIBRATE_TELEMETRY] Livestream URL (starts after selecting a calibration): "
            f"{_orange_text(stream_url)}"
        )
    else:
        print("[CALIBRATE_TELEMETRY] Livestream disabled (--no-livestream).")
